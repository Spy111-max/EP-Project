...
export default function AQISummaryGrid({ cards, loading, onCardClick }) {
  if (!loading && (!cards || cards.length === 0)) {
    return (
      <section className="border border-slate-300 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <div className="border-b border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-950/40">
          ...
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Select a city to view PM2.5, PM10, and PM1 summary cards.</p>
        </div>
      </section>
    );
  }
